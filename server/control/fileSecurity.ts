...
export const allowedEvidenceMimeTypes = ["application/pdf", "image/jpeg", "image/png", "image/webp"] as const;
...
export function assertEvidenceFileInput(input: { contentType: string; sizeBytes: number }) {
  if (!(allowedEvidenceMimeTypes as readonly string[]).includes(input.contentType)) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "Only PDF, JPG, PNG, and WebP evidence files are permitted." });
  }
