...
import { CheckCircle2, ExternalLink, History, Image as ImageIcon, Paperclip, ShieldCheck, Upload } from "lucide-react";
...
function AttachmentLink({ attachment }: { attachment: { id: string; originalName: string; contentType: string; sizeBytes: number } }) {
  ...
  const file = trpc.control.exceptions.getNoteAttachment.useQuery({ organisationId: scope.organisationId, attachmentId: attachment.id }, { enabled: isImage });
  const open = async () => {
    const result = await file.refetch();
    if (result.data?.url) window.open(result.data.url, "_blank", "noopener,noreferrer");
  };
  if (isImage) return <button type="button" onClick={open} disabled={file.isFetching} className="group flex max-w-[11rem] items-center gap-2 rounded-xl border bg-white p-1.5 text-left transition-colors hover:border-teal-300 hover:bg-teal-50 disabled:opacity-60"><span className="grid size-9 shrink-0 place-items-center overflow-hidden rounded-lg bg-slate-100">{file.data?.url ? <img src={file.data.url} alt="" className="size-full object-cover" /> : <ImageIcon className="size-4 text-slate-500" />}</span><span className="min-w-0"><span className="block truncate text-[11px] font-bold text-slate-700">{attachment.originalName}</span><span className="block text-[10px] text-muted-foreground">{file.isFetching ? "Loading preview…" : t("previewAttachment")}</span></span><ExternalLink className="mr-1 size-3 shrink-0 text-teal-700 opacity-0 transition-opacity group-hover:opacity-100" /></button>;
...
function InvestigationNotes({ exceptionId, className = "" }: { exceptionId: string; className?: string }) {
  ...
  const [uploadProgress, setUploadProgress] = useState<{ phase: "preparing" | "uploading"; completed: number; total: number } | null>(null);
