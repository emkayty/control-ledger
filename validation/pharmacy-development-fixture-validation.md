...
The control also maintains a capped, browser-memory **Local simulation log** of the most recent ten simulated failures. An entry records only the selected synthetic failure mode, configured local delay, and local display time. It contains no patient, prescription, batch, stock, user, credential, provider, request, audit, or live operational content. The **Clear log** action is now always visible: it is disabled when the history is empty and, when enabled, clears only the current browser-memory entries. The focused test covers both states, including clearing an offline simulation entry without invoking a fixture callback or a service. The log disappears automatically on browser refresh. Mobile review confirmed that the visible disabled control, delay field, and empty log remain compact; the recorded-error list is otherwise covered through the simulator’s focused component tests rather than a live browser interaction.
