...
| Automated regression coverage | 17 Vitest files and 56 tests passed after the latest source changes. | Strong targeted coverage; not a replacement for full end-to-end, load, or penetration testing. |
...
The final local suite contains 56 passing tests across access control, router contracts, reconciliation, association corrections, evidence-file boundaries, receipt proposal parsing, protected extraction, approval state transitions, portfolio aggregation, component behavior, and the new governance regressions. The suite is valuable because it tests exact behavior and negative paths rather than merely rendering pages.
...
| **High** | Exception audit events are now written in the same database transaction as submission/approval decisions, but a real database rollback-on-audit-failure integration test is still missing. Other material workflows may still use audit writes after state writes. | A ledger needs strong confidence that a material state cannot survive without its audit record. | Expand transactional audit writes across all material operations and test actual transaction rollback against a real test database. |
